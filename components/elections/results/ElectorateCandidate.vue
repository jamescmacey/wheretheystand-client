            ...
            <div class="flex-shrink-0 w-60">
                <div class="flex items-center space-x-2">
                    <div class="flex-shrink-0 w-2 self-stretch flex items-center justify-center rounded-md"
                        :style="{ backgroundColor: electionDataStore.getPersistentPartyForCandidate(election.slug, resultsVersion.slug, {id: result.candidate, number: Number(result.candidate)})?.colour || 'gray' }">
                    </div>
                    <div class="flex flex-col">
                        {{ electionDataStore.getCandidateForCandidate(election.slug, resultsVersion.slug, {id: result.candidate, number: Number(result.candidate)})?.name || ('Unknown candidate' + ' (' + result.candidate.slice(-4) + ')') }}
                        <p class="text-muted text-sm uppercase">{{ electionDataStore.getPartyForCandidate(election.slug, resultsVersion.slug, {id: result.candidate, number: Number(result.candidate)})?.name || 'Independent' }}</p>
                    </div>
                </div>
            </div>
            <div class="flex-grow">
